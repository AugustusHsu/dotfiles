...
| `Alt+Z` | 縮放/還原終端機面板（開一個蓋滿整個畫面的浮動視窗，編輯器、側邊欄都會被蓋住；再按一次還原；n / t / i 模式都能按） |

`Ctrl+/` 開關的對象是**最後使用的終端機**：預設是 `claude`，但只要透過 `Ctrl+t` 切換過其他終端機，之後 `Ctrl+/` 開關的就是那個，不會強制跳回 `claude`。

`Alt+Z` 縮放的也是這個「最後使用的終端機」，做法是開一個浮動視窗接到同一個終端機 buffer，不會動到 edgy 管理的版面本身（曾經試過直接關掉編輯器/側邊欄視窗，但 edgy 偵測到「main 區域視窗全部消失、只剩邊欄視窗」會反過來連終端機也關掉，卡住無法恢復）。開關、切換、新增終端機都會自動把縮放狀態重設回正常大小，不會殘留到下一個顯示出來的終端機上。選 `Alt+Z` 而不是 `Ctrl+Z`／`Ctrl+\`：前者是終端機內建的 job control（暫停前景程式），後者會跟 nvim 終端機模式內建的跳出鍵 `<C-\><C-n>` 衝突。
